// LinearSectionEntryCsvImporter.cs
// Place this file inside an "Editor" folder in your Unity project
// (e.g. Assets/Editor/LinearSectionEntryCsvImporter.cs)
// Requires SoundEntryCsvImportUtils.cs in the same Editor folder.
//
// Usage: Unity menu -> Tools -> Music -> Import Linear Sections from CSV
// Imports the "LinearSections" CSV into LinearSectionEntry ScriptableObjects.
//
...
public static class LinearSectionEntryCsvImporter
{
    private const string LogPrefix = "LinearSectionEntryCsvImporter";
    private const string OutputFolder = "Assets/TrackEntries/Linear";

    [MenuItem("Tools/Music/Import Linear Sections from CSV")]
    public static void ImportFromCsv()
    {
        string path = OpenCsvPanelAndReadRows("Select Linear Sections CSV", out List<string[]> rows);
        if (path == null)
            return;
        ...
        Debug.Log($"[{LogPrefix}] Done. Created {created} LinearSectionEntry assets in '{OutputFolder}' " +
